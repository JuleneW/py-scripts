...
outside_temp = read_thingspeak(2)
room_lights = read_thingspeak(3)
room_temp = read_thingspeak(4)
...
def handle_switches():
    global adjusting_room_lights, room_lights, room_temp, last_mode_switch_time
    while running:
        ...
        if wiringpi.digitalRead(pin_switch0) == 0:  # input is active low
            ...
            if adjusting_room_lights:
                ...
                print("----------------------------")
                print(f'temperature changed to: {room_temp} °C')
        ...
        if wiringpi.digitalRead(pin_switch1) == 0:  # input is active low
            ...
            if adjusting_room_lights:
                ...
                room_temp = float(numpy.clip(room_temp - 0.5, 0, 50))
                print("----------------------------")
                print(f'temperature changed to: {room_temp:.1f} °C')
...
try:
    ...
    while True:
        ...
        MQTT_DATA = f"field1={sens_brightness_value}&field2={sens_temperature_value:.1f}&field3={room_lights}&field4={room_temp:.1f}"
        try:
        ...
        print(f'Outside temp: {outside_temp:.1f} °C')
        ...
        lcd_1.clear()
        lcd_1.go_to_xy(0, 0)
        lcd_1.put_string(f'{brightness:.0f} lux')
        lcd_1.go_to_xy(0, 10)
        lcd_1.put_string(f'{outside_temp:.1f} °C')
        lcd_1.go_to_xy(0, 20)
        lcd_1.put_string(f'{room_lights:.0f} %')
        lcd_1.go_to_xy(0, 30)
        lcd_1.put_string(f'{room_temp:.1f} °C')
