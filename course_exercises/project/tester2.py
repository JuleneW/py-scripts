...
def read_thingspeak_WAARDES(field):
    ...
    try:
        ...
        print('N/A')
    return 0
room_1_WAARDE = read_thingspeak_WAARDES(1)
room_2_WAARDE = read_thingspeak_WAARDES(2)
room_3_WAARDE = read_thingspeak_WAARDES(3)
room_4_WAARDE = read_thingspeak_WAARDES(4)
room_5_WAARDE = read_thingspeak_WAARDES(5)
room_6_WAARDE = read_thingspeak_WAARDES(6)
room_7_WAARDE = read_thingspeak_WAARDES(7)
room_8_WAARDE = read_thingspeak_WAARDES(8)
def read_thingspeak_GEWENST(field):
    ...
    try:
        ...
        print('N/A')
    return 0
room_5_GEWENST = read_thingspeak_GEWENST(5)
# Other variables
threshold = 22
...
def handle_switches():
    ...
    while running:
        ...
        if wiringpi.digitalRead(pin_switch0) == 0:  # input is active low
            ...
            room_5_GEWENST = float(numpy.clip(room_5_GEWENST + 1, 0, 50))
            print("----------------------------")
            print(f'Change temperature to: {room_5_GEWENST:.1f} °C')
            # Publish updated value to ThingSpeak
            MQTT_DATA_GEWENST = f"field5={room_5_GEWENST:.1f}"
...
try:
    ...
    while True:
        ...
        try:
            ...
            client.reconnect()
        print("----------------------------")
        print(f'My room temp: {room_5_WAARDE:.1f} °C')
        print(f'My desired room temp: {room_5_GEWENST:.1f} °C')
        ...
        lcd_1.go_to_xy(0, 10)
        lcd_1.put_string(f'Temp: {room_5_WAARDE:.1f} °C')
        lcd_1.go_to_xy(0, 20)
        lcd_1.put_string(f'--> {room_5_GEWENST:.1f} °C')
        # Motor control logic ONLY based on ThingSpeak temp
        if room_5_WAARDE > threshold and screens_state == 'open':
            lcd_1.go_to_xy(0, 40)
        ...
        elif room_5_WAARDE <= threshold and screens_state == 'closed':
            lcd_1.go_to_xy(0, 40)
        ...
        print("----------------------------")
        print("Overview all rooms current value:")
        print(f'R1 = {room_1_WAARDE:.1f} °C     R2 = {room_2_WAARDE:.1f} lux')
        print(f'R3 = {room_3_WAARDE:.1f} lux    R4 = {room_4_WAARDE:.1f} °C')
        print(f'R5 = {room_5_GEWENST:.1f} °C    R6 = {room_6_WAARDE:.1f} lux')
        print(f'R7 = {room_7_WAARDE:.1f} lux    R8 = {room_8_WAARDE:.1f} lux')
        lcd_1.refresh()
        DeactivateLCD()    
        # PushOver message to phone
        if room_5_WAARDE > threshold:
